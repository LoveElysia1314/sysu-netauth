...
def _adapter_metadata() -> dict[str, dict]:
    ...
    try:
        decoded = _run_powershell(
            "Get-NetAdapter | "
            "Select-Object Name,InterfaceDescription,HardwareInterface,MediaConnectState | "
...
def list_candidates() -> list[InterfaceCandidate]:
    ...
    for name, stat in stats.items():
        ...
        # ── 物理链路状态 ──
        # Windows MediaConnectState: 1=Connected, 2=Disconnected, 3=Unknown
        # 元数据缺失时以 psutil speed > 0 作为 fallback
        media_connect = meta.get("MediaConnectState")
        if media_connect == 1:
            has_media = True
            reasons.append("media")
        elif media_connect is not None:
            has_media = False
            reasons.append("no-media")
        else:
            # 元数据缺失（Win7/精简版等），以 speed > 0 作为近似判定
