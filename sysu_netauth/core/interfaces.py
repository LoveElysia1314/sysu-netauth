...
def _win32_enumerate() -> list[dict]:
...
def has_physical_media_psutil() -> bool:
    """Use psutil only (no PowerShell) to check if any physical Ethernet
    interface has an active link.

    这是 _has_any_media / list_ethernet_candidates 的兜底方案。
    过滤规则集中在 _name_is_virtual / _is_virtual_mac / WIRELESS_PATTERNS，
    与 _resolve_interface_type 保持一致。
    """
    stats = psutil.net_if_stats()
    addrs = psutil.net_if_addrs()
    for name, stat in stats.items():
        if _name_is_virtual(name):
            continue
        if WIRELESS_PATTERNS.search(name):
            continue
        if not stat.isup or not stat.speed or stat.speed <= 0:
            continue
        mac = _get_mac_from_addrs(addrs.get(name, []))
        if mac is not None and _is_virtual_mac(mac):
            continue
        return True  # 通过所有过滤
    return False


def _get_mac_from_addrs(addr_list: list) -> str | None:
    """Extract the MAC address from a psutil address list, or return None."""
...
def _name_is_virtual(name: str, description: str = "") -> bool:
    ...
    Shared by _resolve_interface_type and has_physical_media_psutil,
    so filtering rules stay in one place.
    """
