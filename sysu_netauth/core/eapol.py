...
def detect_reauth_trigger(raw: bytes) -> bool:
    """Check if a raw EAPOL frame is a reauthentication trigger from the switch.

    Returns True if the frame is:
      - EAP-Request/Identity  (switch wants identity)
      - EAP-Request/MD5       (switch wants MD5 challenge)

    Designed to be called from a background sniffing listener.
    """
    parsed = parse_eapol(raw)
    if not parsed:
        return False
    if parsed.code == EAP_REQUEST and parsed.eap_type in (EAP_IDENTITY, EAP_MD5):
        return True
    return False
