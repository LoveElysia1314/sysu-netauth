...
SERVICE_START_TYPE_AUTO = "auto"
SERVICE_START_TYPE_DEMAND = "demand"
...
def _icon_path(name: str) -> Path:
    return resolve_asset_path(name)


TRAY_STATES = ("idle", "success", "error", "warn")
...
class CampusTray:
    ...
    def _poll_service_status(self) -> None:
        ...
        if stale and status.state not in {"stopped", "idle"}:
            self.set_state("error", "服务无响应")
        ...
        if status.state != self._last_service_state:
            ...
            if status.state == "authenticated" and self.config.desktop_notify:
            ...
            elif status.state == "waiting_network" and self.config.desktop_notify:
                self.tray.showMessage(
                    APP_DISPLAY_NAME,
                    f"网络已断开：{message}",
                    self.icons["warn"],
            ...
            elif status.state == "failed" and self.config.desktop_notify:
                self.tray.showMessage(
                    APP_DISPLAY_NAME,
                    f"认证失败：{message}",
                    self.icons["error"],
    ...
    def _apply_config_change(self, new_config: AppConfig) -> None:
        ...
        if new_config.service_mode != old_service_mode:
            if new_config.service_mode:
                _adjust_service_start_type(SERVICE_START_TYPE_AUTO)
            else:
                _adjust_service_start_type(SERVICE_START_TYPE_DEMAND)
                _ensure_service_running()
    ...
    def _on_user_authenticate(self) -> None:
        ...
        self.set_state("warn", "已请求重新连接")

    def logoff(self) -> None:
        self._flush_quick_form()
        write_command("logoff")
        self.set_state("warn", "已请求断开连接")
